...
def get_head_img(friends):
    ...
    for i, uname in enumerate(username):
        with open("D:\\picture\\headImgs" + str(i) + ".png", "wb") as f:
...
def create_img():
    x = 0
    y = 0
    imgs = os.listdir("D:\\picture\\")  # 返回 headImgs 目录下的文件列表
    ...
    width = int(math.sqrt(input_length * input_width / len(imgs)))   # 以 长*宽 来拼接图片，math.sqrt()开平方根计算每张小图片的宽高，这里设定每一张小图片还是正方形，虽然存在一点偏差，头像数量越多，壁纸右边和下边的空白越少
    num_line = int(input_width / width)   # 每行图片数

    for i in imgs:  #对每一张图片逐个进行处理
        try:
            img = Image.open("D:\\picture\\headImgs/" + i + ".png")
        except IOError:
